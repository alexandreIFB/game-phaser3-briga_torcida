...
class GameEstacionamento extends Phaser.Scene {
  player
  stars
  bombs
  platforms
  ...
  create() {
    ...
    this.estacionamento = this.add.image(400, 300, 'estacionamento');
    ...
    if (this.player.anims.currentAnim.key == 'kick') {
      ...
      if (this.enemy.alive === false) {
        ...
        if (this.enemy2?.alive === false || this.enemy2?.alive === undefined) {
          ...
          if (this.counter >= 2) {
            this.physics.pause();
            var config = this.add.bitmapText(320, 330, 'carrier_command', 'level 3 completed', 12);
            var level2 = this.add.bitmapText(320, 370, 'carrier_command', 'start level 4', 12);
...
export default GameEstacionamento